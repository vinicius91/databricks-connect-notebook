...
# the viniro user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the viniro user to the desired the user (NB_USER)
    if id viniro &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" viniro 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the viniro user:"
            _log "- username: viniro       -> ${NB_USER}"
            _log "- home dir: /home/viniro -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the viniro user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the viniro home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "viniro" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/viniro to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/viniro/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/viniro to /home/${NB_USER}!"
                _log "Attempting to symlink /home/viniro to /home/${NB_USER}..."
                if ln -s /home/viniro "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/viniro to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/viniro/"* ]]; then
...
# user we started as.
    ...
    viniro_UID="$(id -u viniro 2>/dev/null)"  # The default UID for the viniro user
    viniro_GID="$(id -g viniro 2>/dev/null)"  # The default GID for the viniro user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old viniro user to nayvoj ($(id -u viniro):$(id -g viniro))"
            ...
            sed --expression="s/^viniro:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/viniro:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "viniro" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/viniro. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/viniro ]]; then
        _log "WARNING: no write access to /home/viniro. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
